...
- A listing's **description** is at most 500 characters: the form counts them and stops there, and the server refuses a longer one. Listings written before the limit keep theirs until they are next edited, and can still be restocked.
...
- **Your orders from this farmer.** Like the order card in a Shopee chat, a buyer sees at the top of the conversation their latest order from that farmer - product photo, quantity, total, date and status, coloured as on My Orders - and **All orders** opens the rest. Each one opens that order's page. The farmer doesn't see this panel.
...
- Endpoints (buyers and farmers only): `GET /api/chats`, `GET /api/chats/unread`, `POST /api/chats` `{ farmerId }` (buyers), `GET /api/chats/:id`, `PATCH /api/chats/:id/read`, `POST /api/chats/:id/messages` `{ text }` or multipart `{ image, text? }`, `GET /api/chats/:id/orders` (buyers), `GET /api/chat-images/:file`. Over the live connection the server sends `chat:message`, `chat:read`, `chat:seen`, `chat:typing` and `chat:presence`, and the browser sends `chat:typing`.
- A person's messages are included in `GET /api/auth/me/export` (photos are marked `photo: true`), and deleting an account deletes their conversations with every message and photo in them.
