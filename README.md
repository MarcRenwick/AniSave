...
- A person's messages are included in `GET /api/auth/me/export` (photos are marked `photo: true`, messages deleted for everyone `deleted: true`), and deleting an account deletes their conversations with every message and photo in them.
